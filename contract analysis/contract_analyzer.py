...
with col2:
    if request:
        completions = openai.ChatCompletion.create(
            engine="gpt4demo",
            messages= [{"role":"system","content":"You are helpful contract analyzer agent\""}, 
               
               {"role":"user","content": contract + request}],
            max_tokens=2000,
            n=1,
            stop=None,
            temperature=0,
    ...
    if user_input:
        completions = openai.ChatCompletion.create(
            ...
            messages= [{"role":"system","content":"You are helpful contract analyzer agent\""}, 
                       ...
                       {"role":"user","content": contract + user_input}],
            max_tokens=2000,
            n=1,
            stop=None,
            temperature=0,
    ...
    if st.button('Generate template'):
        completions = openai.ChatCompletion.create(
            ...
            messages= [{"role":"system","content":"You are helpful contract analyzer agent\""}, 
               ...
               {"role":"user","content": "Generate a Service Delivery Agreement with the following elements: Service Provider: {service_provider}, Client: {client}, Description of Services: {services_description}, Start Date: {start_date}, Duration: {duration}"}
               ],
