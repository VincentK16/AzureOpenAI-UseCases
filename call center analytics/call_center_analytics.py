...
def create_ticket(data):
    response = openai.ChatCompletion.create(
      engine="gpt4demo",
        messages=[
            {"role": "system", "content": "You are a customer service representative for an insurance company. You are tasked with creating a ticket for a customer who called in to report a dent on his car. "},
            {"role": "user", "content": transcript+"Generate a response email to the transcript above, notifying the customer that the ticket has been created and apologizing if it was complaining. The name of the customer is {data['name']"
            }],
...
def generate_email(transcript):
    response = openai.ChatCompletion.create(
      ...
      messages=[
            {"role": "system", "content": "You are a customer service representative for an insurance company. You are tasked with creating a ticket for a customer who called in to report a dent on his car. "},
            {"role": "user", "content": transcript + "Generate a response email to the transcript above, notifying the customer that the ticket has been created and apologizing if it was complaining. The name of the customer is {data['name']} and the policy number is {data['policy_number']"
            },
        ],
...
def improvement(data):
    response = openai.ChatCompletion.create(
      engine="gpt4demo",
        messages=[
            {"role": "system", "content": "You are a customer service representative for an insurance company. You are tasked with creating a ticket for a customer who called in to report a dent on his car. "},
            {"role": "user", "content": transcript + "Elaborate a list of remediations to get to the following improvement: {data['contact_center_improvement']"}
        ],
